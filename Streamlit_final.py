...
def delete_poll(poll_id, vendor_id):
    ...
    if poll_index is not None:
        ...
        message = f"🗑️ **POLL DELETED**: The poll for {removed_poll['quantity']} {removed_poll['unit']} of {removed_poll['product']} has been deleted."
        add_message_to_community(
...
with st.sidebar:
    ...
    if st.session_state.current_user:
        ...
        if st.button("My Supply Commitments", use_container_width=True):
            st.session_state.view = "My Supply Commitments"
            st.rerun()
...
if not st.session_state.current_user:
    ...
    if st.session_state.view == "communities":
    ...
    elif st.session_state.view == "my_supply_commitments":
        # My Active Polls View (for both farmers and vendors)
        st.subheader("My Supply Commitments")
        ...
        if st.session_state.current_user_type == "vendor":
            ...
            st.divider()
            st.subheader("My Closed Poll Contributions")
...
# Add a footer
st.divider()
st.write("FarmConnect App - Connecting Farmers and Vendors")
st.write("© 2025 FarmConnect. All rights reserved.")